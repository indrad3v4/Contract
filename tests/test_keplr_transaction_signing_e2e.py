...
class MockKeplrWallet:
    ...
    async def signAmino(self, chainId, signer, signDoc, signOptions=None):
        ...
        for msg in signDoc["msgs"]:
            ...
            if "type" not in msg:
                raise ValueError(f"Expected a message object with 'type' field, but got {msg}")
                
            if "value" not in msg:
                raise ValueError(f"Expected a message object with 'value' field, but got {msg}")
            ...
            if msg["type"] == "cosmos-sdk/MsgSend":
                ...
                for field in required_fields:
                    if field not in value:
                        raise ValueError(f"MsgSend value missing required field: {field}")
...
class TestKeplrTransactionSigningE2E:
    ...
    async def test_transaction_signing_flow(self, mock_fetch, mock_window, mock_keplr):
        ...
        signResult = await mock_fetch("/api/sign", {
            "method": "POST",
        ...
        # Assertions
        assert len(mock_keplr.sign_calls) == 1, "signAmino should be called exactly once"
    ...
    async def test_match_keplr_documentation(self, mock_keplr):
        ...
        # Assert
        assert success, f"Signing with Keplr docs example failed: {error if 'error' in locals() else 'unknown error'}"
