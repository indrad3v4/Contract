...
def mock_js_convert_amino_to_proto(amino_msg):
    ...
    logger.debug(
        f"Converting Amino message type '{amino_msg['type']}' to Proto typeUrl '{type_url}'"
    )
...
class MockKeplrWallet:
    ...
    def validate_message(self, msg):
        ...
        if "typeUrl" not in msg:
            raise ValueError(
                "Expected a message object with typeUrl, but got: " + str(msg)
            )
...
class TestKeplrFrontendCompatibility:
    ...
    def test_js_conversion_function(self):
        ...
        assert self.keplr.validate_message(
            proto_msg
        ), "Keplr should accept the converted message"
    ...
    def test_main_js_sign_doc_conversion(self):
        ...
        # Verify Keplr would accept this format
        assert self.keplr.validate_sign_doc(
            proto_doc
        ), "Keplr should accept the converted sign doc"
    ...
    def test_field_name_conversion(self):
        ...
        # Verify conversion
        assert (
            converted_fields == proto_fields
        ), "Field names should be properly converted"
...
class TestRealKeplrErrorScenarios:
    ...
    def test_mixed_format_error(self):
        ...
        fixed_doc = {
            **mixed_doc,
            "msgs": [
                ...
                mock_js_convert_amino_to_proto(
                    mixed_doc["msgs"][1]
                ),  # Fix the second one
            ],
        }
        ...
        assert self.keplr.validate_sign_doc(
            fixed_doc
        ), "Document with all Proto messages should be valid"
