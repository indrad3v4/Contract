...
class TestKeplrMessageFormat:
    ...
    def test_amino_to_proto_conversion(self):
        ...
        assert proto_doc['msgs'][0]['value']['fromAddress'] == expected_proto['msgs'][0]['value']['fromAddress']
        assert proto_doc['msgs'][0]['value']['toAddress'] == expected_proto['msgs'][0]['value']['toAddress']
        assert proto_doc['msgs'][0]['value']['amount'] == expected_proto['msgs'][0]['value']['amount']
    ...
    def test_keplr_message_format_requirements(self):
        ...
        assert keplr_msg['typeUrl'].startswith('/cosmos.'), "typeUrl must start with /cosmos."
    ...
    def test_improper_message_format_detection(self):
        ...
        with pytest.raises(AssertionError):
            assert 'fromAddress' in amino_msg['value'], "Should fail: fromAddress not in Amino message"
    ...
    def test_sign_doc_format_compatibility(self):
        ...
        assert msg['typeUrl'] == '/cosmos.bank.v1beta1.MsgSend', "Incorrect typeUrl value"
        ...
        with pytest.raises(AssertionError):
            assert 'fromAddress' in amino_msg['value'], "Should fail: fromAddress not in Amino value"
