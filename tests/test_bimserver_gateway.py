...
def mock_bimserver_gateway():
    ...
    with patch('src.gateways.bimserver_gateway.BIMServerGateway._authenticate') as mock_auth:
        ...
        gateway.headers = {'Content-Type': 'application/json', 'Authorization': f'Bearer {SAMPLE_TOKEN}'}
        return gateway
...
class TestBIMServerGateway:
    ...
    def test_authentication_failure(self, mock_error_response):
        ...
        with patch('requests.post', return_value=mock_error_response):
            ...
            assert "Authentication failed" in str(excinfo.value) or "Failed to authenticate with BIMserver" in str(excinfo.value)
    ...
    def test_store_file(self, mock_bimserver_gateway, mock_successful_response):
        ...
        with patch('requests.post', return_value=mock_successful_response), \
             patch.object(mock_bimserver_gateway, '_get_deserializer_by_name', return_value='ifc-deserializer'):
            ...
            with patch.object(mock_bimserver_gateway, '_call_api', side_effect=responses):
    ...
    def test_retrieve_file(self, mock_bimserver_gateway, mock_successful_response):
        ...
        with patch('requests.post', return_value=mock_successful_response), \
             patch.object(mock_bimserver_gateway, '_get_serializer_by_name', return_value='ifc-serializer'), \
             patch.object(mock_bimserver_gateway, '_decode_file_data', return_value=b"test file content"):
    ...
    def test_get_projects(self, mock_bimserver_gateway, mock_successful_response):
        ...
        with patch.object(mock_bimserver_gateway, '_call_api', return_value={'result': [{'id': '1', 'name': 'Project 1'}]}):
    ...
    def test_get_revisions(self, mock_bimserver_gateway, mock_successful_response):
        ...
        with patch.object(mock_bimserver_gateway, '_call_api', return_value={'result': [{'id': '1', 'comment': 'First revision'}]}):
...
class TestStorageFactory:
    ...
    def test_create_bimserver_gateway_when_enabled(self):
        ...
        with patch('flask.current_app.config.get', side_effect=lambda key, default=None: config_values.get(key, default)), \
             patch('src.gateways.bimserver_gateway.BIMServerGateway.__init__', return_value=None), \
    ...
    def test_fallback_to_local_storage_when_bimserver_fails(self):
        ...
        with patch('flask.current_app.config.get', side_effect=lambda key, default=None: config_values.get(key, default)), \
             patch('src.gateways.bimserver_gateway.BIMServerGateway.__init__', side_effect=Exception("BIMserver connection failed")):
