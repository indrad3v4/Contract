...
class OpenAIBIMAgent:
    ...
    def identify_stakeholder(self, messages: List[Dict]) -> Optional[str]:
        ...
        try:
            ...
            system_message = (
                ...
                "based on their messages: Tenant/Buyer, Broker, Landlord, Property Manager, Appraiser, "
                "Mortgage Broker, or Investor. Return only the stakeholder type as a single word or phrase."
            )
    ...
    def process_message(self, message: str, bim_data: Optional[Dict] = None) -> Tuple[str, Dict]:
        ...
        try:
            ...
            if self.bim_data:
                bim_context = f"BIM data available: {self.bim_data.get('summary', 'None')}"
                system_message += f"\n\n{bim_context}"
            
            # Add stakeholder context if identified
            if self.identified_stakeholder:
                stakeholder_name = StakeholderGroup.get_name(self.identified_stakeholder)
                stakeholder_context = f"The user appears to be a {stakeholder_name}. Tailor your responses accordingly."
                system_message += f"\n\n{stakeholder_context}"
            ...
            metadata = {
                ...
                "stakeholder_name": StakeholderGroup.get_name(self.identified_stakeholder) if self.identified_stakeholder else None,
                "model": "gpt-4o" if self.enhanced_mode else "gpt-3.5-turbo",
            }
